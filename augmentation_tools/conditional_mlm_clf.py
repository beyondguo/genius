"""
Conditional-MLM Aug for Classification
...
# load mlm model
print(f'c-mlm model: {args.mlm_model_path}')
...
class LabelPrefixTokenizedDataset:
    def __init__(self, contents, labels):
        self.contents = []
        for content,label in zip(contents,labels):
            prompt = label2desc[label] + ': '
            self.contents.append(prompt+content)
...
tokenized_dataset = LabelPrefixTokenizedDataset(contents, labels)
...
if args.output_name is None:
    args.output_name = f"cmlm_{args.mlm_model_path.split('/')[-1]}_p{args.p}_top{args.topk}_aug{args.n_aug}"
