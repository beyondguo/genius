...
from ner_and_qa.easy_text_augmenter import TextAugmenter

TA = TextAugmenter(lang='en', using_wordnet=True)
...
for _ in range(N_AUG):
    for c in m_pre_contexts:
        aug_c = TA.aug_by_replacement(c,p=0.1)
        generated_pre_contexts.append(' '.join(aug_c))

generated_post_contexts = []
for _ in range(N_AUG):
    for c in m_post_contexts:
        aug_c = TA.aug_by_replacement(c,p=0.1)
        generated_post_contexts.append(' '.join(aug_c))
...
f_name = f'qa_data/squad_first{N_TRAIN}_aug{N_AUG}_SR.pkl'
df.to_pickle(f_name)
print(f_name)