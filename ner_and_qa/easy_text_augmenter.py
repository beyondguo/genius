...
class TextAugmenter:
    ...
    # given模式：直接插入给定的n个词
    ########################################################################
    def aug_by_insertion(self, text, p, mode='random', given_words=[], print_info=False):
        words = self.tokenizer(text)
        n = max(1, int(p * len(words)))
        ...
        if mode == 'random':
            ...
            if mode == 'selective':
                # selected_words不一定都是本文中的，可能是给定的一个大集合，所以我们要先筛选出在本文中的词
                given_words = list(set(given_words).intersection(set(words)))
            random.shuffle(given_words)
            if n > len(given_words):  # 当given_words的数量不够时，用random来凑
                for given_word in given_words:
                    word_to_insert = self.add_word(new_words, mode, given_word)
                    insertion_res.append(word_to_insert)
                for i in range(n - len(given_words)):
                    word_to_insert = self.add_word(new_words, 'random')
                    insertion_res.append(word_to_insert)
            else:  # 否则，只插入n个
                for i in range(n):
                    word_to_insert = self.add_word(new_words, mode, given_words[i])
                    insertion_res.append(word_to_insert)
        if print_info:
...
if __name__ == "__main__":  # for test
    ta = TextAugmenter('en',using_wordnet=True)
